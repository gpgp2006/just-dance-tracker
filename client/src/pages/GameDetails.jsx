    ...
    <div className="container mt-5">
      <div className="d-flex align-items-center justify-content-between mb-4">
        ...
        <div className="form-check form-switch bg-white p-2 rounded border shadow-sm ps-5 pe-3">
            ...
            />
            <label className="form-check-label fw-bold user-select-none" htmlFor="editModeSwitch" style={{cursor: 'pointer'}}>
          ...
          <div className="card-body py-3">
              <div className="row">
                  <div className="col-md-12 mb-2">
                          ...
                          {(GAME_PLATFORMS[edition] || []).map(platform => (
                              <button
                                  ...
                                  style={{
                                      width: '50px', 
                                      height: '50px',
                                      border: filterPlatform === platform ? '2px solid #0d6efd' : '1px solid #dee2e6',
                                      backgroundColor: filterPlatform === platform ? 'white' : '#f8f9fa',
                                      opacity: filterPlatform && filterPlatform !== platform ? 0.5 : 1,
                      ...
                      {filterPlatform ? (
                          <small className="text-primary fw-bold mt-1 d-block">{filterPlatform}</small>
                      ) : (
                          <small className="text-muted mt-1 d-block fst-italic">Visualização Geral (Mostrando a melhor pontuação de todas as plataformas)</small>
                      )}
                  </div>

                  <div className="col-md-6 mt-3">
                      <label className="form-label small text-muted fw-bold text-uppercase">Filtrar por Método</label>
                      <select 
                          className="form-select"
                          value={filterMethod}
                          onChange={(e) => setFilterMethod(e.target.value)}
                          disabled={!filterPlatform || getValidInputs(filterPlatform, edition).length <= 1}
                      >
                          <option value="">
                              {getValidInputs(filterPlatform, edition).length <= 1 && filterPlatform 
                                  ? filterMethod 
                                  : "Todos os Métodos"
                              }
                          </option>
                          {filterPlatform && getValidInputs(filterPlatform, edition).length > 1 && getValidInputs(filterPlatform, edition).map(input => (
              ...
              {songs.length > 0 ? (
                songs.map((song) => (
                  <SongRow 
                    ...
                    // Passa o usuário real para o SongRow usar no salvamento
                    userSettings={{
                        default_platform: filterPlatform || user?.default_platform || "",
                        default_input: filterMethod || user?.default_input || ""
                    }}
                    currentUser={user} // Passa o objeto do usuário inteiro
                  />
                ))
              ) : (
                <tr>
