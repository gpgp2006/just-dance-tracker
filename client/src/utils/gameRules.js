// client/src/utils/gameRules.js

// Definição base dos inputs suportados por cada console em GERAL
...
// Nova função inteligente que pede a edição para decidir os inputs
export const getValidInputs = (platform, edition) => {
    // Converte edição para número para garantir
    const editionNum = parseInt(edition);

    // Verifica se existe uma exceção para essa edição e plataforma
    if (EXCEPTIONS[editionNum] && EXCEPTIONS[editionNum][platform]) {
        return EXCEPTIONS[editionNum][platform];
    }

    // Se não tiver exceção, retorna o padrão da plataforma
